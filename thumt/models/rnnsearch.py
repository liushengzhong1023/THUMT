...
def _copy_through(time, length, output, new_output):
    '''
    If time >= length, use output; else, use new_output
    Typically, output is the zero_output; new_output is the cell_output.
    '''
...
def _gru_encoder(cell, inputs, sequence_length, initial_state, dtype=None):
    ...
    time_steps = tf.shape(inputs)[1]  # the length of each sentence, a single value
    ...
    # Function for GRU at one time-step / word position
    def loop_func(t, out_ta, state):
        ...
        # Write value into index position of the TensorArray.
...
def _encoder(cell_fw, cell_bw, inputs, sequence_length, dtype=None,
    ...
    with tf.variable_scope(scope or "encoder",
        ...
        # return a map containing different result fields
        # output: [batch, word_sequence, output_size]
        # annotation: [batch, word_sequence, 2 * output_size]
        # final state: [batch, hidden_unit]
        results = {
...
def _decoder(cell, inputs, memory, sequence_length, initial_state, dtype=None,
    ...
    # Assume that the underlying cell is GRUCell-like
    batch = tf.shape(inputs)[0]
    time_steps = tf.shape(inputs)[1]  # the total sequence length, actually may be shorter
    dtype = dtype or inputs.dtype
    output_size = cell.output_size
    zero_output = tf.zeros([batch, output_size], dtype)
    ...
    with tf.variable_scope(scope or "decoder", dtype=dtype):
        ...
        # cache is generated by no query and no bias
        ...
        def loop_func(t, out_ta, att_ta, val_ta, state, cache_key):
            ...
            # apply attention to the input, state is the query, memory is the input for attentions
            ...
            # feed both input of target word and the context
        ...
        # output array, attention weight, value array (aka context)
        ...
        # output array and context value array
        output_final_ta = outputs[1]
        value_final_ta = outputs[3]

        # output shape: [batch, sequence length, output size]
        ...
        # context shape: [batch, sequence length, context size]
        ...
        # output, context, and initial state (last hidden state of encoder)
        result = {
...
def model_graph(features, mode, params):
    ...
    # define the two GRU cells for the encoder
    ...
    # Inference: return the distribution among target vocabulary words, class distribution.
    if mode == "infer":
        # Special case for non-incremental decoding; only preserve the last time-step
        # Only infer the last word
        maxout_features = [
    ...
    # maxout layer + first linear layer
    maxhid = layers.nn.maxout(maxout_features, maxout_size, params.maxnum, concat=False)
    readout = layers.nn.linear(maxhid, params.embedding_size, False, False, scope="deepout")
    ...
    # Prediction, predict a specific word
    logits = layers.nn.linear(readout, tgt_vocab_size, True, False, scope="softmax")
    ...
    # different computation for training loss and evaluation loss
    if mode == "eval":
...
class RNNsearch(NMTModel):
    ...
    def get_training_func(self, initializer, regularizer=None, dtype=None):
        ...
        If using half-precision training, all variables are in tf.float16 type; else, no constraint on variable dtype.
        '''
    ...
    def get_inference_func(self):
        def inference_fn(features, params=None):
            '''
            From the features --> log probabilities. Return the inference function.
            The input of the inference_fn is the features, while the output is the log_prob.
            '''
            if params is None:
                params = copy.copy(self.parameters)
            else:
