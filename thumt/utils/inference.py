...
def _beam_search_step(time, func, state, batch_size, beam_size, alpha,
                      pad_id, eos_id):
    '''
    The state is updated in this function, which means newly generated beams are updated.
    The sequence lengh increases by 1.
    '''
    ...
    # func is generated by _get_inference_fn above, which needs two parameters: inputs, state
    # inputs is the current output sequence, or the candidate sequences
...
def beam_search(func, state, batch_size, beam_size, max_length, alpha,
    ...
    '''
    Instead of greedily choosing the most likely next step as the sequence is constructed, the beam search expands all
    possible next steps and keeps the k most likely, where k is a user-specified parameter and controls the number of
    beams or parallel searches through the sequence of probabilities.
    "Keep track of k states rather than just one. Start from random generation. At each step, all successors of all k
    states are generated. Then selects the k best successors from the complete list and repeats."
    '''
...
def create_inference_graph(models, features, params):
    '''
    beam_size is used for beam search in decoding.
    '''
    ...
    # inference_fn: features --> log_prob, only predict for the last word
    ...
    for model_fn in model_fns:
        if callable(model_fn):
            # For non-incremental decoding (e.g., RNNSearch)
            states.append({})
    ...
    # get the index in vocabulary for pad, bos, and eos
    ...
    # For source sequence length. [batch] --> [batch, beam_size]
